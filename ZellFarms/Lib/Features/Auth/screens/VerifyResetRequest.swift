...
struct VerifyResetRequest: View {
    let email: String
    ...
    var body: some View {
        VStack {
            Text("Enter the 5-digit Password Reset OTP sent to \(email)")
            ...
            CustomButton(
                action: {
                    Task {
                        await authRepo.verifyResetRequest(
                            emails: email,
                            otp: combinedOTP
                        )
                    }
        ...
        .padding(15)
        .navigationTitle("Password Reset OTP")
        .onAppear {
