...
struct DeleteOTPScreen: View {
    ...
    @State private var email = ZelPreferences.email
    ...
    var body: some View {
        VStack {
            Text("Enter the 5-digit Delete Account OTP sent to \(email)")
            ...
            CustomButton(
                action: {
                    Task {
                        await authRepo.deleteAccount(otp: combinedOTP)
                    }
                },
                text: "Delete Account"
            )
        ...
        .padding(15)
        .navigationTitle("Delete Account OTP")
        .onAppear {
