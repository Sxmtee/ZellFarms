...
struct HomeCheapestSeeAll: View {
    let cheapestList: [ProductCheapest]
    ...
    var body: some View {
        VStack {
            ScrollView(showsIndicators: false) {
                LazyVGrid(columns: layout, spacing: 15) {
                    ForEach(cheapestList, id: \.id) { choice in
                        HomeProductCard(
        ...
        .navigationTitle("All Cheapest Product")
    }
}
